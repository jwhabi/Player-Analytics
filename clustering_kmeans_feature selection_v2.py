# -*- coding: utf-8 -*-
"""
Created on Wed Apr 19 22:10:11 2018
...
train_new1 = train_new[['defensive_work_rate','preferred_foot','attacking_work_rate','gk_diving', 'standing_tackle']].copy()
train_new1.shape

test_new1 = test_new[['defensive_work_rate','preferred_foot','attacking_work_rate','gk_diving', 'standing_tackle']].copy()
test_new1.shape

from sklearn.cluster import KMeans
kmeans1 = KMeans(n_clusters=4, random_state=0)
kmeans1.fit(train_new1)
test['kmeans_2vars'] = kmeans1.predict(test_new1)


x=test.kmeans_2vars[test.position=='gk']
x.value_counts() # 0 for 1

y=test.kmeans_2vars[test.position=='def']
y.value_counts() # 3 for 2

z=test.kmeans_2vars[test.position=='mid']
z.value_counts()# 1 for 3

a=test.kmeans_2vars[test.position=='for']
a.value_counts() # 2 for 4

#Assignment of labels to cluster based on domain knowledge
test['predicted5'] =np.where((test['kmeans_2vars']==0), 10,test['kmeans_2vars'])
test['predicted5'] =np.where((test['kmeans_2vars']==3), 20,test['predicted5'])
test['predicted5'] =np.where((test['kmeans_2vars']==1), 30,test['predicted5']) 
test['predicted5'] =np.where((test['kmeans_2vars']==2), 40,test['predicted5'])
...
print(accuracy_score(test.predicted5,test.position2))#66.52
print(precision_score(test.predicted5,test.position2,average=None)) #[ 0.99612676  0.85908763  0.33489923  0.82216463]
print(recall_score(test.predicted5,test.position2,average=None)) #[ 0.99577614  0.63034994  0.60940301  0.66473758]
print(f1_score(test.predicted5,test.position2,average=None)) #[ 0.99595142  0.72715479  0.4322527   0.73511724]
